...
### ★ 5/10 — `Scenario.tokens` vs `letters.json` — redundancja danych

**Pliki:** `src/types/index.ts` (pole `tokens`), `src/scenarios/droga-donikad/letters.json`
**Problem:** `Scenario.tokens: Record<string, number>` (np. `{"A": 30, "B": 83}`) i `letters.json` (`{letters: [{id: "a", paragraphId: "30"}]}`) istnieją niezależnie. Wartości w `tokens` to liczby żetonów na planszy — niekoniecznie numery paragrafów. Jednak oba pola opisują ten sam fizyczny element gry (żetony alfabetu). `tokens` jest używany **wyłącznie** w `ScenariosList.tsx` do wyświetlania statystyk. Przed dodaniem edycji `tokens` do edytora warto ustalić czy to pole nie powinno być wyliczane z `letters.json` (np. liczba liter per typ) lub w ogóle usunięte.
**Działanie:** Zbadać czy `tokens` da się zastąpić danymi z `letters.json` i czy jest potrzebne jako osobne pole.
**Ryzyko:** niskie (używane tylko do wyświetlania). **Nakład:** ~1 h analizy + ewentualna migracja.
